...
def extract_create_assert_serialization(data_provider, block_class: Type[Block]):
    data = extract_block_data(data_provider, block_class)
    block = create_block_object(data, block_class)
    assert_serialization_to_json(block, data[block_class.get_associated_block_type().value])
